    ...
    */
    if( false === $args['show_posts_links'] ) {
        ...
        if( 1 === $max_pages ) {
            ...
            */
            foreach ( $range_numbers as $v ) {
                ...
                if ( $v == $current_page ) {
                    ...
                    $page_numbers[] = '<a href="' . get_pagenum_link( $v ) . '" class="inactive" aria-label="Post Page '.$v.    '">' . $v . '</a>';
            ...
            */
            $previous_page  = ( $current_page !== 1 )                       ? '<a href="' . get_pagenum_link( $current_page - 1 ) . '" aria-label="Previous Post Page">' . $args['previous_page_text'] . '</a>' : '';
            $next_page      = ( $current_page !== $max_pages )              ? '<a href="' . get_pagenum_link( $current_page + 1 ) . '" aria-label="Next Post Page">' . $args['next_page_text'] . '</a>'     : '';
            ...
            */
            ...
            $paginated_text = '<div class="pagination" aria-label="Navigating Posts">';
        ...
        */
        $paginated_text = get_next_posts_link( '<div class="next-posts-link" aria-label="Next Post Link">' . $args['next_link_text'] . '</div>', $max_pages );
        $paginated_text .= get_previous_posts_link( '<div class="previous-posts-link" aria-label="Previous Post Link">' . $args['previous_link_text'] . '</div>' );
